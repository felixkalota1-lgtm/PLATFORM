...
export default function Inquiries({ items, history = [], onGeneratePDF, onSendEmail, onDeleteHistory }: InquiriesProps) {
  const [selectedHistoryId, setSelectedHistoryId] = React.useState<string | null>(null)
  const [previewId, setPreviewId] = React.useState<string | null>(null)
  
  React.useEffect(() => {
    console.log(`Inquiries component received - items: ${items.length}, history: ${history.length}`)
  }, [items, history])
      ...
      <div style={{ flex: 1, padding: '32px', overflow: 'auto' }}>
        {items.length === 0 && history.length === 0 ? (
          <div style={{ textAlign: 'center', paddingTop: '60px', color: '#64748b' }}>
            <p style={{ fontSize: '16px', fontWeight: '500', margin: '0 0 8px 0' }}>No inquiries yet</p>
            <p style={{ fontSize: '13px', margin: '0', color: '#94a3b8' }}>Select products from the inventory and click "Add to Inquiry" to create one</p>
          </div>
        ) : (
          <div>
            {items.length > 0 && (
              <>
                <div style={{ marginBottom: '28px' }}>
                  <h3 style={{ margin: '0 0 18px 0', fontSize: '18px', fontWeight: '700', color: '#5b7c99' }}>Inquiry Items ({items.length})</h3>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: '15px', color: '#5b7c99', fontWeight: '700', marginTop: '12px', paddingTop: '12px', borderTop: '1px solid #d0dce6' }}>
                        Inquiry Total: {formatCurrency(calculateTotal(), items[0]?.currency)}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {history && history.length > 0 && (
              <div style={{ marginTop: '28px' }}>
                <h3 style={{ margin: '0 0 18px 0', fontSize: '18px', fontWeight: '700', color: '#5b7c99' }}>Previous Inquiries ({history.length})</h3>
