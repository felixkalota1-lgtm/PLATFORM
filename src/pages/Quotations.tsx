...
export default function Quotations({
  ...
  React.useEffect(() => {
    console.log(
      `Quotations component received - items: ${items.length}, history: ${history.length}`,
    );
  }, [items, history]);
            ...
            <button
              onClick={() => {
                if (window.confirm("Delete this quotation?")) {
                  // Clear current items by calling with empty trigger
        ...
        {items.length === 0 && history.length === 0 ? (
          ...
          >
            ...
            <p style={{ fontSize: "13px", margin: "0", color: "#94a3b8" }}>
              Select products from the inventory and click "Add to Quotation" to
              create one
            </p>
          </div>
        ) : (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "flex-end" }}>
                    <div style={{ textAlign: "right" }}>
                      ...
                      >
                        Quotation Total:{" "}
                        {formatCurrency(calculateTotal(), items[0]?.currency)}
                      </div>
                    </div>
                  </div>
                </div>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              Total:{" "}
                              {formatCurrency(
                                histItem.items.reduce(
                                  (sum: number, item: any) =>
                                    sum + item.price * item.qty,
                                  0,
                                ),
                                histItem.items[0]?.currency,
                              )}
                            </p>
                          </div>
                        </div>
