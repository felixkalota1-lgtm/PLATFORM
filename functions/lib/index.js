 ...
 */
export const sendEmailViaGmail = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const message = buildEmailMessage(account.email, toEmail, subject, htmlBody, attachmentData);
 ...
 */
export const fetchInboxEmails = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", `Failed to fetch inbox: ${error.message}`);
    }
});
/**
...
export const markEmailAsRead = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", `Failed to mark email as read: ${error.message}`);
    }
});
/**
 * Helper: Build MIME email message
 */
function buildEmailMessage(from, to, subject, htmlBody, attachmentData) {
        ...
        for (const [userId, userData] of Object.entries(users)) {
            ...
            for (const [accountId, account] of Object.entries(emailAccounts)) {
                try {
                    ...
                    console.error(`Failed to sync inbox for user ${userId} account ${accountId}:`, error);
                    failed++;
                }
            }
        }
